...
const Home = () => {
  ...
  const dataFunction = async (userData) => {
    if (userData) {
      ...
      try {
        const collectionData = await databaseService.get_collections_id(
          user_id
        );
        if (collectionData) {
          // fetching income....
          ...
          const categoryData =
            await databaseService.get_planned_categoryWise_exp(
              plannedCategory_id
            );
          if (categoryData) setPlanned(categoryData);

          // actual expense data...
          const actual_exp_id = collectionData.actual_exp_id;
          const expenseData = await databaseService.get_actual_exp(
            actual_exp_id
          );
            ...
            <div className="w-1/3">
              <div className="w-full p-4 mt-4 bg-gray-100 border-2 rounded-lg border-gray-300">
                ...
                <h3 className="text-xl text-green-700">
                  Saving: {currentIncome - totalExp} /-
                </h3>
                <br />
                <h3 className="text-xl text-center text-gray-700 mb-2">
                  Expenses in each categories:{" "}
                </h3>
                <p className="text-gray-600 ">
                  Fixed (Rent, Insurance, Education, Loan): {fixed} /-
                </p>
